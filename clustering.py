"""clustering.py — Universe-aware, lane-aware, priority-weighted memory clustering.

Pure computation module — no I/O, no imports from memory.py.
Dependencies: numpy, scipy

Renamed from 3am_clustering.py (leading digit breaks importlib.import_module).
Feature flag `use_new_clusterer` removed — this IS the clusterer.
"""
    ...
    def health_report(
        ...
        if oversized:
            warnings.append(
                ...
                f"— consider increasing adjustment_factor in config.json (currently {factor})"
            )
        ...
        if tiny > n_clusters * 0.3:
            warnings.append(
                ...
                f"— consider decreasing adjustment_factor in config.json (currently {factor})"
            )
